...
const COURSE_NAME_ALIASES: Record<string, CoreCourseName> = {
...
export function normalizeCourseName(value: string): string {
  ...
  return COURSE_NAME_ALIASES[cleanedValue] ?? cleanedValue;
}

export function isCoreCourseName(
  value: string
): value is CoreCourseName {
  return CORE_COURSES.includes(value as CoreCourseName);
}
...
export function getCoreCourseName(
  ...
  return isCoreCourseName(normalizedValue)
    ? normalizedValue
    : null;
}